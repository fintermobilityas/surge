...
void print_table(const std::vector<ReleaseEntry>& releases) {
    ...
    // Print header
    std::cout << fmt::format("  {:<{}}  {:<{}}  {:>{}}  {:>{}}  {:<{}}  {:<{}}", h_ver, w_ver, h_chan, w_chan, h_full,
                             w_full, h_delta, w_delta, h_date, w_date, h_notes, w_notes)
              << "\n";

    // Print separator
    std::cout << "  " << std::string(w_ver, '-') << "  " << std::string(w_chan, '-') << "  " << std::string(w_full, '-')
              << "  " << std::string(w_delta, '-') << "  " << std::string(w_date, '-') << "  "
              << std::string(w_notes, '-') << "\n";
    ...
    for (const auto& r : releases) {
        ...
        std::cout << fmt::format("  {:<{}}  {:<{}}  {:>{}}  {:>{}}  {:<{}}  {:<{}}", r.version, w_ver, ch_str, w_chan,
                                 format_size(r.full_size), w_full, delta_str, w_delta, r.created, w_date, r.notes,
                                 w_notes)
                  << "\n";
    }
}
