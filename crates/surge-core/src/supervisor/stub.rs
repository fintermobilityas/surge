//! Stub executable: finds and launches the latest installed application version.
...
/// Scan the install directory for `app-*` subdirectories and return the path
/// to the one with the highest version number.
///
/// Expects directories named `app-{version}` (e.g., `app-1.0.0`, `app-2.1.3`).
/// Returns an error if no valid app directories are found.
pub fn find_latest_app_dir(install_dir: &Path) -> Result<PathBuf> {
    ...
    match best_dir {
        ...
        None => Err(SurgeError::NotFound(format!(
            "No app-* directories found in {}",
            install_dir.display()
        ))),
    }
}

/// Find the latest installed version and exec into it.
///
/// Scans `install_dir` for `app-*` directories, selects the highest version,
/// and replaces the current process with the application executable.
///
...
/// # Arguments
///
/// * `install_dir` - The root installation directory containing `app-*` subdirectories
...
/// 1. A file with the same name as the directory (minus `app-` prefix)
