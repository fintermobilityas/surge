...
bool atomic_rename(const std::filesystem::path& src,
    ...
    spdlog::debug("atomic_rename: rename failed ({}), falling back to copy+remove",
                  ec.message());
...
bool copy_directory(const std::filesystem::path& src,
    ...
    std::filesystem::copy(src, dst,
        ...
        std::filesystem::copy_options::overwrite_existing, ec);
    if (ec) {
        spdlog::error("copy_directory: failed to copy '{}' -> '{}': {}",
...
bool remove_directory(const std::filesystem::path& dir) {
    ...
    std::filesystem::remove_all(dir, ec);
    if (ec) {
        spdlog::error("remove_directory: failed to remove '{}': {}",
...
std::filesystem::path create_temp_dir(std::string_view prefix) {
    ...
    std::string suffix;
    for (int i = 0; i < 8; ++i) suffix.push_back(hex[dist(gen)]);
    ...
    if (ec) {
        throw std::runtime_error(
            fmt::format("Failed to create temp directory: {}", ec.message()));
    }
    return result;
#else
    ...
    if (!result) {
        throw std::runtime_error(
            fmt::format("mkdtemp failed: {}", strerror(errno)));
    }
...
bool set_permissions(const std::filesystem::path& path, uint32_t mode) {
    ...
    if (chmod(path.c_str(), static_cast<mode_t>(mode)) != 0) {
        spdlog::error("set_permissions: chmod failed on '{}': {}",
...
bool make_executable(const std::filesystem::path& path) {
    ...
    if (stat(path.c_str(), &st) != 0) {
        spdlog::error("make_executable: stat failed on '{}': {}",
                      path.string(), strerror(errno));
        return false;
    }
    mode_t new_mode = st.st_mode | S_IXUSR | S_IXGRP | S_IXOTH;
    if (chmod(path.c_str(), new_mode) != 0) {
        spdlog::error("make_executable: chmod failed on '{}': {}",
                      path.string(), strerror(errno));
        return false;
    }
    return true;
