...
fn main() {
    ...
    let generated = payload::generate(work_dir, args.scale, args.seed).expect("Failed to generate payloads");
    ...
    if !args.skip_diff {
        ...
        let chunked_patch_result = runner::run_chunked_bspatch(&first_archive, &chunked_patch, archive_v2.len() as u64);
        log!(
            json_mode,
            "  {} {}",
    ...
    if !args.skip_diff {
        log!(json_mode, "");
        log!(json_mode, "Update scenario (delta chain)...");
        let scenario_results = runner::run_update_scenario(&generated.v1_dir, &generated.v2_dir, args.num_deltas);
