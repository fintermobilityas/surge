...
pack:                             # optional; omitted uses built-in defaults
  delta:
    ...
    max_chain_length: 8
    chunked_patch_format: 1       # 1 = readable by every client (default); 2 = identity-chunk bitset; 3 = per-chunk target digests; 4 = SURGPAT1 fast-apply payloads (each needs clients that know its format)
  compression:
    format: zstd
    level: 3
  retention:
