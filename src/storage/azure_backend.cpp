...
std::vector<uint8_t> base64_decode(std::string_view input) {
    ...
    for (char c : input) {
        ...
        bits += 6;
        if (bits >= 8) { bits -= 8; result.push_back(static_cast<uint8_t>((buf >> bits) & 0xFF)); }
    }
    return result;
}
...
AzureSharedKeySigner::AzureSharedKeySigner(std::string_view account_name,
                                            std::string_view account_key)
    : account_name_(account_name)
    , account_key_decoded_(base64_decode(account_key)) {}
...
std::string AzureSharedKeySigner::rfc1123_now() { return rfc1123_date(); }
...
struct AzureBlobBackend::Impl {
    ...
    std::string build_canonical_headers(const std::map<std::string, std::string>& headers) const {
        ...
        for (auto& [k, v] : headers) {
            if (k.starts_with("x-ms-")) result += fmt::format("{}:{}\n", k, v);
        }
        return result;
    }

    std::string build_shared_key_auth(
        const std::string& method, const std::string& blob_name,
        ...
        std::string string_to_sign = fmt::format(
            ...
            build_canonical_headers(headers), account, container, blob_name);
        auto signature = crypto::hmac_sha256(account_key_decoded_,
...
AzureBlobBackend::AzureBlobBackend(const StorageConfig& config)
    ...
    if (impl_->account.empty()) {
        if (const char* env = std::getenv("AZURE_STORAGE_ACCOUNT")) impl_->account = env;
    }
...
int32_t AzureBlobBackend::get_object(const std::string& key, std::vector<uint8_t>& out_data) {
    ...
    struct curl_slist* hdr_list = nullptr;
    for (auto& [k, v] : headers) hdr_list = curl_slist_append(hdr_list, fmt::format("{}: {}", k, v).c_str());
...
int32_t AzureBlobBackend::head_object(const std::string& key, ObjectInfo& out_info) {
    ...
    out_info.key = key;
    if (auto it = resp_headers.find("content-length"); it != resp_headers.end()) out_info.size = std::stoll(it->second);
    if (auto it = resp_headers.find("etag"); it != resp_headers.end()) out_info.etag = it->second;
    if (auto it = resp_headers.find("last-modified"); it != resp_headers.end()) out_info.last_modified = it->second;
    return SURGE_OK;
}
...
int32_t AzureBlobBackend::list_objects(const std::string& prefix, ListResult& out_result,
