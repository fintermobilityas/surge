...
int cmd_demote(int argc, char* argv[]) {
    ...
    // Step 2: Parse release index and remove the channel from the specified version
    // TODO: Parse the release index YAML, find the entry for `version`,
    // remove `channel` from its channels list.
    //
    ...
    //         found = true;
    //         YAML::Node new_channels;
    //         for (const auto& ch : release["channels"]) {
    //             if (ch.as<std::string>() != channel) {
    //                 new_channels.push_back(ch);
    //             }
    //         }
    //         release["channels"] = new_channels;
    //         break;
    //     }
    // }
    // if (!found) {
    //     spdlog::error("Version {} not found in release index", version);
    //     return 1;
    // }
    ...
    spdlog::info("Version {} removed from channel '{}'", version, channel);
